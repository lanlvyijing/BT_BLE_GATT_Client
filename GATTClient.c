...
void timer_cb(VM_TIMER_ID_NON_PRECISE tid, void* user_data)
{
    ...
    vm_timer_delete_non_precise(tid);//关闭定时器
}
...
{
	// initialize a Bluetooth module and register a callback for various connection events
	//0x00080000 | 0x00000001 开启BT以及BLE
    vm_bt_cm_init(app_client_bt_init_cb,0x00080000 | 0x00000001,NULL);
    if (VM_BT_CM_POWER_ON == vm_bt_cm_get_power_status())//Gets the current power status of the Bluetooth.
    {
        app_client_init();
    }
    else
...
void app_client_init(void)
{
    ...
    if(g_appc_cntx.state == APPC_STATUS_DISABLED)///* context of client application */
    {
